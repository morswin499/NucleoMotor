...
void HAL_TIM_MspPostInit(TIM_HandleTypeDef* timHandle)
{
  ...
  /* USER CODE END TIM1_MspPostInit 0 */
    ...
    PA11     ------> TIM1_CH4
    */
    GPIO_InitStruct.Pin = PWM_D_Pin|PWM_MB_Pin|PWM_MC_Pin|PWM_MA_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void Int32ArrayToBytes(const int32_t values[4], uint8_t out[16])
{
    union {
        int32_t i;
        uint8_t b[4];
    ...
    for (int i = 0; i < 4; i++)
    {
        converter.i = values[i];
        out[i * 4 + 0] = converter.b[0];
        out[i * 4 + 1] = converter.b[1];
        out[i * 4 + 2] = converter.b[2];
        out[i * 4 + 3] = converter.b[3];
    }
}
